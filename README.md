...
- Local dev: if you don't want to use Pinecone, you can adapt the vectorstore to use Chroma only 
- Increase `chunk_size`/`chunk_overlap` in `ingestion.py` to tune retrieval granularity.
- The project uses `gpt-4o-mini` (model configured in `core.py`) — swap for another provider/model if needed.
